...
// 176 184
int min_radius_refine = 176;
int max_radius_refine = 184 ;
int param1 = 100;
int param2 = 20;
int ratioMinDist = 2;
...
static void CannyThreshold(int, void*) {
    ...
    for (int i = 0; i < images.size(); i++) {
        ...
        // Find the circle
        std::vector<Vec3f> circles_plate;
        std::vector<Vec3f> circles_salad;
        std::vector<Vec3f> actual_plates;
        std::vector<Vec3f> refine_salad;

        HoughCircles(gray_transformed, circles_plate, HOUGH_GRADIENT, 1, gray_transformed.rows / ratioMinDist, param1, param2, min_radius_hough_plates, max_radius_hough_plates);
        HoughCircles(gray_transformed, circles_salad, HOUGH_GRADIENT, 1, gray_transformed.rows / ratioMinDist, param1, param2, min_radius_hough_salad, max_radius_hough_salad);
        HoughCircles(gray_transformed, refine_salad, HOUGH_GRADIENT, 1, gray_transformed.rows / ratioMinDist, param1, param2, min_radius_refine, max_radius_refine);
        ...
        for (size_t i = 0; i < circles_salad.size(); i++) {
            for (size_t j = 0; j < circles_plate.size(); j++) {
                Vec3i s = circles_salad[i];
                Vec3i p = circles_plate[j];
                cv::Point center_salad = Point(s[0], s[1]);
                cv::Point center_plate = Point(p[0], p[1]);
                if (cv::norm(center_plate - center_salad) < p[2]) {
                    std::vector<Vec3f>::iterator it = actual_plates.begin()+j;
        ...
        if (actual_plates.size() > 2) {
            ...
            for (size_t i = 0; i < actual_plates.size(); i++) {
                for (size_t j = 0; j < refine_salad.size(); j++) {
                    Vec3i s = actual_plates[i];
                    Vec3i p = refine_salad[j];
                    cv::Point center_salad = Point(s[0], s[1]);
                    cv::Point center_plate = Point(p[0], p[1]);
                    if (cv::norm(center_plate - center_salad) < p[2]) {
                        std::vector<Vec3f >::iterator it = actual_plates.begin()+i;
                        actual_plates.erase(it);
                    }
                }
            }
    ...
    imshow(window_plates, imageGrid);e
...
int main( int argc, char** argv )
{
    ...
    createTrackbar("min:", window_src, &min_radius_refine, 300, CannyThreshold);
    createTrackbar("max:", window_src, &max_radius_refine, 300, CannyThreshold);
